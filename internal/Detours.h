...
//nvse's DetourVtable has no ownership check or rollback
class VtableDetour {
	UInt32 slotAddr = 0;
	UInt32 overwritten_addr = 0;
	UInt32 installedAddr = 0;
public:
	bool Write(UInt32 slot, UInt32 dst) {
		if (slotAddr || !slot || !dst)
			return false;

		const UInt32 original = *reinterpret_cast<UInt32*>(slot);
		if (!original)
			return false;

		DWORD oldProtect;
		if (!VirtualProtect((void*)slot, sizeof(UInt32), PAGE_EXECUTE_READWRITE, &oldProtect))
			return false;

		overwritten_addr = original;
		slotAddr = slot;
		installedAddr = dst;
		*reinterpret_cast<UInt32*>(slot) = dst;
		VirtualProtect((void*)slot, sizeof(UInt32), oldProtect, &oldProtect);
		MemoryBarrier();
		return true;
	}

	template<typename T>
	bool Write(UInt32 slot, T dst) {
		return Write(slot, (UInt32)dst);
	}

	UInt32 GetOverwrittenAddr() const { return overwritten_addr; }
	bool IsInstalled() const { return slotAddr != 0; }
	bool OwnsPatch() const {
		return slotAddr != 0 && *reinterpret_cast<UInt32*>(slotAddr) == installedAddr;
	}
	...
	bool Remove() {
		...
		DWORD oldProtect;
		if (!VirtualProtect((void*)slotAddr, sizeof(UInt32), PAGE_EXECUTE_READWRITE, &oldProtect))
			return false;

		*reinterpret_cast<UInt32*>(slotAddr) = overwritten_addr;
		VirtualProtect((void*)slotAddr, sizeof(UInt32), oldProtect, &oldProtect);
		MemoryBarrier();
